...
Milo Reporter — фронтальный 2.5D cat mascot по пояс: 31 upper-body bone, 71 активная RGBA-часть, полные arm chains `shoulder → upperArm → elbow → forearm → wrist → paw`, закрытые и открытые лапы, 21 emotional viseme и 10 reporter clips. Базовые raster-parts хранят `intrinsicSize / aspectLocked`; Replace artwork использует contain-fit без одностороннего растяжения.
```

Human и Dog сразу дают редактируемый skeleton, визуальные части/texture slots и базовый набор `idle / walk / run / jump / fall / land`. Milo Reporter использует дискретные `attachment`-треки: ключ содержит `partId | null`, допускает только `step / hold`, а runtime показывает один attachment на slot и при crossfade переключает его на входящий клип при весе 50%. Его Face workspace управляет левым/правым expression и pupil slot независимо или linked; зрачки движутся отдельными `eyeAim` bones, радужки повторяют взгляд с меньшим parallax и остаются под прозрачной окантовкой век внутри отдельной safe-zone каждой формы глаза. Общая маска лица не используется. Там же можно записать гримасу на playhead и сгенерировать потоковую viseme-последовательность до конца клипа с регулируемым шагом; reporter clips синхронно анимируют полные цепи рук и лап. Все три шаблона сохраняются и экспортируются в нативных source/runtime-форматах Bones. Внешние CC0-модели, rig-файлы, анимации и PNG-наборы используются только как референсы анатомии и движения.
