Ниже — технический документ для разработки **Bones**: собственного визуального редактора векторного силуэтного персонажа и анимаций под **PixiJS 8 + 2D web platformer + LDtk**.

> **Source of truth.** Этот документ является главным источником истины и конечной целью разработки Bones. Любая задача по реализации, рефакторингу, тестированию, документации или архитектурному решению должна сверяться с этим документом до начала работ. Если задача не уточняет поведение явно, нужно брать намерение, границы и терминологию отсюда.
>
> **Prompt context protocol.** Перед выполнением новой задачи исполнитель должен:
>
> 1. Найти в `doc.md` минимальный релевантный раздел по теме задачи.
> 2. Извлечь из него продуктовый смысл, ограничения, целевые API/форматы и запреты.
> 3. Дополнить рабочий контекст задачи этими пунктами.
> 4. Реализовать только текущий этап, не забегая в будущие разделы.
> 5. Если задача конфликтует с `doc.md`, явно отметить конфликт и следовать последнему прямому указанию пользователя.
>
> **Milestone prompts.** `implementation-milestones.md` описывает порядок этапов. Каждый промт этапа должен трактоваться как краткая инструкция, которую нужно расширять релевантным контекстом из `doc.md`, а не как замена этому документу.

# Technical Design Document

## Bones: Custom Vector Rig Animation Editor for PixiJS Platformers

Название проекта: **Bones**.
...
Milo Reporter — фронтальный 2.5D cat mascot по пояс: 31 upper-body bone, 71 активная RGBA-часть, полные arm chains `shoulder → upperArm → elbow → forearm → wrist → paw`, закрытые и открытые лапы, 21 emotional viseme и 10 reporter clips. Авторитетная пустая основа морды хранится отдельно от ушей, век, радужек, зрачков и mouth-attachments; базовые raster-parts хранят `intrinsicSize / aspectLocked`, а Replace artwork использует contain-fit без одностороннего растяжения.
```

Human и Dog сразу дают редактируемый skeleton, визуальные части/texture slots и базовый набор `idle / walk / run / jump / fall / land`. Milo Reporter использует дискретные `attachment`-треки: ключ содержит `partId | null`, допускает только `step / hold`, а runtime показывает один attachment на slot и при crossfade переключает его на входящий клип при весе 50%. Его Face workspace управляет левым/правым expression и pupil slot независимо или linked; зрачки движутся отдельными `eyeAim` bones, радужки повторяют взгляд с меньшим parallax, а активная форма века включает собственную aperture-mask чуть шире прозрачного проёма и отсекает радужку со зрачком за его границей. Общая маска лица не используется. Там же можно записать гримасу на playhead и сгенерировать потоковую viseme-последовательность до конца клипа с регулируемым шагом; reporter clips синхронно анимируют полные цепи рук и лап. Все три шаблона сохраняются и экспортируются в нативных source/runtime-форматах Bones. Внешние CC0-модели, rig-файлы, анимации и PNG-наборы используются только как референсы анатомии и движения.
